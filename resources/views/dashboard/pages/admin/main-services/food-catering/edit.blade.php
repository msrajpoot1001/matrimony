...
@section('title', 'Dashboard | Edit Food Catering')
        ...
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Edit Food Catering</h4>
                </div>

                <div class="card-body">
                    <form action="{{ route('admin.food-catering.update', $item->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="banner__inputlist">
                                        <select name="looking_for" class="form-control" required>
                                            @foreach (['Marriage', 'Thread Ceremony', 'House Warming', 'Birthday', '100 Days Celebration', 'Shradha', 'Pinda Dana', 'Puja for Harmony', 'Funeral Rituals', 'Other'] as $option)
                                                <option value="{{ $option }}"
                                                    {{ old('looking_for', $item->looking_for) == $option ? 'selected' : '' }}>
                                                    {{ $option }}
                                                </option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="mt-4">
                            ...
                            <a href="{{ route('admin.food-catering.index') }}" class="btn btn-secondary">
                                Cancel
                            </a>
                        </div>
